-- Fix statement timeout on pipeline_tier_metrics RPC
-- Root cause: DISTINCT ON over 32K+ rows with JSON extraction is too slow
-- Solution: Add index on (contact_id, created_at DESC) for the DISTINCT ON,
-- and add a composite index to speed up the JSON filter.

-- Speed up DISTINCT ON (contact_id) ORDER BY contact_id, created_at DESC
CREATE INDEX IF NOT EXISTS idx_lead_loads_contact_dedup
  ON lead_loads (contact_id, created_at DESC);

-- Speed up WHERE contact_state IS NOT NULL AND priority_context->>'lead_date' IS NOT NULL
CREATE INDEX IF NOT EXISTS idx_lead_loads_state_leaddate
  ON lead_loads (contact_state)
  WHERE contact_state IS NOT NULL
    AND priority_context->>'lead_date' IS NOT NULL;

-- Rewrite RPC: split into fast tier/bucket query + separate call query
-- to avoid LATERAL join timeout on 32K+ rows.
