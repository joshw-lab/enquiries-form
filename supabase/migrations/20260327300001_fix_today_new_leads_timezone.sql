-- Fix: lead_date cast to timestamptz was using UTC midnight instead of AWST midnight.
-- Calls made between 00:00-08:00 AWST (before midnight UTC) failed the join condition,
-- causing called contacts to still appear as "awaiting call".
...
BEGIN
  ...
  SELECT
    ...
    ((ll.priority_context->>'lead_date') || 'T00:00:00+08:00')::timestamptz AS lead_date,
    ll.created_at AS loaded_at,
    fc.first_call_at,
    CASE
      WHEN fc.first_call_at IS NOT NULL
        THEN EXTRACT(EPOCH FROM (fc.first_call_at - ((ll.priority_context->>'lead_date') || 'T00:00:00+08:00')::timestamptz))::double precision
      ELSE NULL
  ...
  LEFT JOIN LATERAL (
    ...
    WHERE cr.hubspot_contact_id = ll.contact_id
      AND cr.call_start >= ((ll.priority_context->>'lead_date') || 'T00:00:00+08:00')::timestamptz
  ) fc ON true
